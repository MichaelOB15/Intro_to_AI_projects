...
public class PlannerAgent extends Agent {
    ...
    private Stack<StripsAction> AstarSearch(GameState startState) {
    	Stack<GameState> path = new Stack<GameState>(); //initialize the path
    	PriorityQueue<GameState> openList = new PriorityQueue<GameState>(); // initialize the open list
    	openList.add(startState); //starting node on here
    	Set<GameState> closedList = new HashSet<GameState>(); // initialize the closed list
    	
    	while(!openList.isEmpty()) { // while the open list is not empty
    		GameState current = openList.remove();
    		//System.out.println(current);// get node with lowest function value
    		closedList.add(current);
    		if(current.isGoal()) { //if we have at least the amount of required wood and gold
    			path.add(current);
    			break;
    		}
    		List<GameState> neighbors = current.generateChildren();// generate successors and set their parents to q
    		//System.out.print(neighbors);
			for(GameState neighbor : neighbors) {// for each succesor
				if(closedList.contains(neighbor)) // if node is in closed list, skip this successor
    				continue;
    			if(openList.contains(neighbor)) { //if location has been seen, update to min cost
    				if(openList.removeIf( (e) -> neighbor.equals(e) && neighbor.getCost() + neighbor.heuristic() <= e.getCost() + e.heuristic())) {
    	...
    	if(path.isEmpty()) {
      	...
      	GameState lastState = path.get(0);

    	Stack<StripsAction> backwardsPath = new Stack<StripsAction>();
    	
    	while (lastState != null) {
    		System.out.println(lastState.prevAction);
    		backwardsPath.add(lastState.prevAction);
